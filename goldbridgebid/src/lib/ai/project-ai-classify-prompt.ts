...
export function buildProjectAiClassifyPrompt(params: {
  ...
  const craftsmanContext = buildCraftsmanCostContext({
    ...
    categories: detectedCategories.slice(0, 4),
    sector: "residential",
  ...
  return {
    system: [
      ...
      "",
      "NO REDUNDANCY RULE (CRITICAL — READ CAREFULLY):",
      "- Each line item must cover a DISTINCT scope of work. NEVER create two items that overlap.",
      "- Combine related sub-tasks into ONE line item.",
      "- Think like a contractor writing a bid: each line item is one billable task, not a sub-step of another task.",
      "- If two potential items could reasonably appear on the same line of a contractor's invoice, merge them.",
      "- Aim for 5–12 distinct line items for a typical residential project. More than 12 usually means you're splitting things too finely.",
      "",
      "SPECIFIC MERGE EXAMPLES (follow these patterns):",
      "  BAD (too granular — DO NOT do this):",
      '    - "Trenching for Electrical Conduit"',
      '    - "Trenching for Sewer Connection"',
      '    - "Trenching for Water Connection"',
      '    - "Site Grading"',
      '    - "Slope for Drainage"',
      '    - "Weed Barrier Installation"',
      '    - "Vapor Barrier Installation"',
      "  GOOD (properly combined):",
      '    - "Utility Trenching & Connections" — covers ALL underground utility work (electrical conduit, sewer line, water line, backfill)',
      '    - "Site Grading & Drainage" — covers grading, slope work, and drainage together',
      '    - "Ground Cover / Barriers" — covers weed barrier, vapor barrier, and any ground membrane work together',
      "",
      "  The rule is: if items share the same TYPE of work (all trenching, all barriers, all grading), combine them into ONE line item.",
      "",
